...
stackedensemble.multinomial.test <- function() {
    ...
    test <- ss[[2]]
    print(summary(train))
    x <- c("CAPSULE","GLEASON","AGE","DPROS","DCAPS","PSA","VOL")
    y <- "RACE"
    nfolds <- 5
    ...
    #expect_lt(stack_mean_per_class_error_train, baselearner_best_mean_per_class_error_train)  #Does not pass in this example, but this is ok
